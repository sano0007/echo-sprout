...
export default function PortfolioOverview({
  ...
  const [sortBy, setSortBy] = useState<'date' | 'amount' | 'progress' | 'impact'>('date');
  ...
  const filteredPurchases = useMemo(() => {
    ...
    if (selectedType !== 'all') {
      filtered = filtered.filter(purchase => purchase.projectType === selectedType);
    }
  ...
  const portfolioBreakdown = useMemo(() => {
    const typeMap = new Map<string, { count: number; credits: number; amount: number; impact: number }>();

    purchases.forEach(purchase => {
      const existing = typeMap.get(purchase.projectType) || { count: 0, credits: 0, amount: 0, impact: 0 };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg shadow-lg p-6">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-blue-100 mt-1">Track your environmental impact investments</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          <div className="text-center">
            <div className="text-2xl font-bold">{stats.totalCarbonOffset.toFixed(1)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Portfolio Breakdown by Type</h3>
            ...
            {portfolioBreakdown.map((item) => (
              <div key={item.type} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-4">
                  <div className={`w-12 h-12 rounded-full bg-gradient-to-r ${getProjectTypeColor(item.type)} flex items-center justify-center text-white text-xl`}>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {item.count} projects • {item.credits.toLocaleString()} credits
                    </p>
                  </div>
                </div>
                <div className="text-right">
          ...
          <div className="space-y-4">
            <div className="text-center p-4 bg-green-50 rounded-lg">
              ...
              <div className="text-xs text-green-600 mt-1">
                Equivalent to {Math.floor(stats.totalCarbonOffset * 2.5)} trees planted
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                ...
                <div className="text-xs text-blue-700">Cars off road (1 year)</div>
              </div>
              <div className="text-center p-3 bg-purple-50 rounded-lg">
                <div className="text-lg font-bold text-purple-600">
                  {Math.floor(stats.totalCarbonOffset * 120)}
                </div>
                <div className="text-xs text-purple-700">Homes powered (1 month)</div>
              </div>
            </div>
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPurchases.map((purchase) => (
            <div key={purchase.id} className="bg-white rounded-lg shadow-lg overflow-hidden">
              {/* Project Header */}
              <div className={`bg-gradient-to-r ${getProjectTypeColor(purchase.projectType)} text-white p-4`}>
                <div className="flex items-center space-x-3">
                  <div className="text-2xl">
                    {getProjectTypeIcon(purchase.projectType)}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-semibold truncate">{purchase.projectName}</h3>
                    <p className="text-sm opacity-90">{purchase.location.country}, {purchase.location.region}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4">
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    <label className="text-xs font-medium text-gray-600">CO2 Offset</label>
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${getVerificationBadge(purchase.verificationStatus)}`}>
                  ...
                  <tr key={purchase.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <div className="font-medium text-gray-800">{purchase.projectName}</div>
                          <div className="text-sm text-gray-600">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-gray-600">{purchase.projectProgress}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
      ...
      {filteredPurchases.length === 0 && (
        <div className="bg-white rounded-lg shadow-lg p-8 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-800 mb-2">No purchases found</h3>
