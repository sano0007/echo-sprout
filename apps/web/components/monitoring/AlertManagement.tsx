...
interface Alert {
  ...
  projectId: string;
  projectName: string;
  alertType: 'progress_reminder' | 'overdue_warning' | 'milestone_delay' | 'impact_shortfall' | 'quality_concern';
...
export default function AlertManagement({
  ...
  const [resolutionModal, setResolutionModal] = useState<{ alertId: string; isOpen: boolean }>({
    alertId: '',
    isOpen: false
  });
  ...
  useEffect(() => {
    let filtered = alerts.filter(alert => {
      ...
      const matchesType = filters.type === 'all' || alert.alertType === filters.type;
  ...
  const getAlertIcon = (type: Alert['alertType'], severity: Alert['severity']) => {
  ...
  const getSeverityBadge = (severity: Alert['severity']) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-medium border ${classes[severity]}`}>
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-red-100 mt-1">
              {activeAlerts.length} active alerts • {criticalAlerts.length} critical • {highAlerts.length} high priority
            </p>
          </div>
          ...
          <div className="divide-y divide-gray-200">
            {filteredAlerts.map((alert) => (
              <div key={alert.id} className={`p-6 hover:bg-gray-50 ${
                ...
                alert.severity === 'medium' ? 'border-l-4 border-yellow-400' :
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="flex justify-end space-x-3 mt-4">
              <button
                onClick={() => setResolutionModal({ alertId: '', isOpen: false })}
