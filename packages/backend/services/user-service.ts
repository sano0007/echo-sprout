...
export class UserService {
  public static async getUserByClerkId(ctx: QueryCtx, clerkId: string) {
    return await ctx.db
      .query('users')
  ...
  public static async getCurrentUser(ctx: MutationCtx | QueryCtx) {
    ...
    const user = await UserService.getUserByClerkId(ctx, identity.subject);
    if (!user) {
  ...
  public static async updateUserHelper(ctx: MutationCtx, data: UserJSON) {
    ...
    if (
      ...
      data.phone_numbers[0].phone_number !== user.phoneNumber
    ) {
