...
export default function ProfilePage() {
    ...
    const userProfiles = {
        creator: {
            ...
            description: "We specialize in large-scale reforestation and renewable energy projects across North America.",
            stats: {
                totalProjects: 12,
                activeProjects: 5,
        ...
        buyer: {
            ...
            description: "Technology company committed to carbon neutrality through strategic carbon credit investments.",
            stats: {
                ...
                <div className="bg-gradient-to-r from-blue-600 to-green-600 text-white p-6 rounded-t-lg">
                    <div className="flex items-center gap-6">
                        <div className="w-24 h-24 bg-white/20 rounded-full flex items-center justify-center">
                            <span className="text-3xl font-bold">{currentProfile.name.charAt(0)}</span>
                        </div>
                        <div className="flex-1">
                            <h1 className="text-3xl font-bold">{currentProfile.name}</h1>
                            <p className="text-blue-100">{currentProfile.userType}</p>
                            <p className="text-blue-100">Member
                                since {new Date(currentProfile.joinDate).toLocaleDateString()}</p>
                        </div>
                                ...
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                <label className="block text-sm font-medium mb-2">Organization
                                                    Name</label>
                                                <input type="text" defaultValue={currentProfile.name}
                                            ...
                                            <div>
                                                ...
                                                <input type="text" defaultValue={currentProfile.location}
                                                       className="w-full p-3 border rounded"/>
                                            </div>
                                            {currentProfile.website && (
                                                <div>
                                                    <label className="block text-sm font-medium mb-2">Website</label>
                                                    <input type="url" defaultValue={currentProfile.website}
                                                           className="w-full p-3 border rounded"/>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    <div>
                                        <h3 className="text-lg font-semibold mb-4">User Type & Role</h3>
                                        <div className="space-y-3">
                                            <label className="flex items-center p-3 border rounded">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-600">Develop and manage carbon
                                                        credit projects</p>
                                                </div>
                                            </label>
                                            <label className="flex items-center p-3 border rounded">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-600">Purchase carbon credits for
                                                        offset goals</p>
                                                </div>
                                            </label>
                                        </div>
                                            ...
                                            {userType === 'creator' ? (
                                                <>
                                                    <div className="bg-blue-50 p-4 rounded text-center">
                                                        <p className="text-2xl font-bold text-blue-600">{currentProfile.stats.totalProjects}</p>
                                                        <p className="text-sm text-gray-600">Total Projects</p>
                                                    </div>
                                                    <div className="bg-green-50 p-4 rounded text-center">
                                                        <p className="text-2xl font-bold text-green-600">{currentProfile.stats.activeProjects}</p>
                                                        <p className="text-sm text-gray-600">Active Projects</p>
                                                    </div>
                                                    <div className="bg-purple-50 p-4 rounded text-center col-span-2">
                                                        <p className="text-2xl font-bold text-purple-600">{currentProfile.stats.creditsGenerated?.toLocaleString()}</p>
                                                ...
                                                <>
                                                    <div className="bg-blue-50 p-4 rounded text-center">
                                                        <p className="text-2xl font-bold text-blue-600">{currentProfile.stats.creditsPurchased?.toLocaleString()}</p>
                                                        <p className="text-sm text-gray-600">Credits Purchased</p>
                                                    </div>
                                                    <div className="bg-green-50 p-4 rounded text-center">
                                                        <p className="text-2xl font-bold text-green-600">${currentProfile.stats.totalSpent?.toLocaleString()}</p>
                                                        <p className="text-sm text-gray-600">Total Invested</p>
                                                    </div>
                                                    <div className="bg-purple-50 p-4 rounded text-center">
                                                        <p className="text-2xl font-bold text-purple-600">{currentProfile.stats.projectsSupported}</p>
                                                        <p className="text-sm text-gray-600">Projects Supported</p>
                                                    </div>
                                                    <div className="bg-orange-50 p-4 rounded text-center">
                                                        <p className="text-2xl font-bold text-orange-600">{currentProfile.stats.co2Offset?.toLocaleString()}</p>
                    ...
                    {activeTab === 'activity' && (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                                {userType === 'creator' ? [
                                    {
                                        action: "Project 'Amazon Reforestation Phase 2' approved",
                                        date: "2024-01-18",
                                        type: "success"
                                    },
                                    {
                                        action: "Monthly report submitted for 'Solar Farm Initiative'",
                                        date: "2024-01-15",
                                        type: "info"
                                    },
                                    {
                                        action: "Document revision requested for 'Wind Power Project'",
                                        date: "2024-01-12",
                                        type: "warning"
                                    },
                                    {action: "New project 'Biogas Plant' registered", date: "2024-01-10", type: "info"}
                                ] : [
                                    {
                                        action: "Purchased 50 credits from 'Amazon Conservation Project'",
                                        date: "2024-01-18",
                                        type: "success"
                                    },
                                    {
                                    ...
                                    {action: "Completed purchase of 30 credits", date: "2024-01-10", type: "success"}
                                ].map((activity, index) => (
                                    <div key={index} className={`p-4 rounded border-l-4 ${
                                        activity.type === 'success' ? 'border-green-500 bg-green-50' :
                                            activity.type === 'warning' ? 'border-yellow-500 bg-yellow-50' :
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="bg-green-50 p-4 rounded border border-green-200">
                                    ...
                                    <p className="text-sm text-green-700 mt-1">Your identity has been verified on
                                        January 10, 2024</p>
                                </div>
                            </div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center p-4 border rounded">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">Uploaded: Jan 8, 2024 • Verified ✓</p>
                                        </div>
