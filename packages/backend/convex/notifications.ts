...
export const sendImmediateAlert = action({
  ...
  handler: async (ctx, { alertId }) => {
    const alert = await ctx.runQuery(
      internal.notifications.getAlertForNotification,
      { alertId }
    );

    if (!alert) {
      console.log(`Alert ${alertId} not found for notification`);
    ...
    await ctx.runMutation(internal.notifications.logNotificationDelivery, {
      alertId,
...
export const sendEscalationNotification = action({
  ...
  handler: async (
    ...
    if (escalatedBy) {
      escalatedByUser = await ctx.runQuery(internal.userService.getUserById, {
        userId: escalatedBy,
      });
    }
    ...
    const templateData = {
      ...
      escalationLevel,
      escalationType,
      escalatedByName: escalatedByUser?.name || 'System',
  ...
  handler: async (ctx, { projectIds }) => {
    ...
    for (const projectId of projectIds) {
      try {
        ...
        const creator = await ctx.runQuery(internal.userService.getUserById, {
...
export const sendWeeklyReport = action({
  ...
  handler: async (ctx, { reportData, recipients }) => {
    ...
    for (const recipient of recipients) {
      const user = await ctx.runQuery(internal.userService.getUserById, {
...
export const sendCustomNotification = mutation({
  ...
  handler: async (
    ...
    // Queue email and SMS delivery if requested
    if (channels.includes('email') || channels.includes('sms')) {
      await ctx.runAction(
        internal.notifications.processCustomNotificationDelivery,
        {
          notificationIds,
          subject,
          message,
...
export const getInAppNotifications = query({
  ...
  handler: async (ctx, { limit = 50, offset = 0, unreadOnly = false }) => {
    const user = await UserService.getCurrentUser(ctx);
    if (!user) {
      throw new Error('Authentication required');
    }
...
async function sendMultiChannelNotification(
  ...
  for (const recipient of options.recipients) {
    ...
    if (recipient.preferredChannels.includes('in_app')) {
      try {
        await createInAppNotification(ctx, recipient, options);
        results.in_app.sent++;
      } catch (error) {
...
async function createInAppNotification(
  ...
  await ctx.runMutation(internal.notifications.createInAppNotification, {
...
async function sendEmailNotification(
  ...
  await ctx.runMutation(internal.notifications.logEmailDelivery, {
    recipientId: recipient.userId,
...
async function sendSMSNotification(
  ...
  await ctx.runMutation(internal.notifications.logSMSDelivery, {
    recipientId: recipient.userId,
...
async function getAlertRecipients(
  ...
  if (alert.projectId) {
    ...
    if (project) {
      ...
      if (project.assignedVerifierId) {
        const verifier = await ctx.runQuery(internal.userService.getUserById, {
          userId: project.assignedVerifierId,
        });
        if (verifier) {
          recipients.push({
            userId: verifier._id,
  ...
  if (alert.severity === 'critical') {
    const admins = await ctx.runQuery(internal.userService.getUsersByRole, {
      role: 'admin',
    });
...
async function getUsersByRoles(
  ...
  for (const role of roles) {
    const roleUsers = await ctx.runQuery(internal.userService.getUsersByRole, {
      role,
    });
    ...
    if (role === 'verifier' && projectId) {
      ...
      if (project && project.assignedVerifierId) {
        const verifier = roleUsers.find(
          (u) => u._id === project.assignedVerifierId
        );
  ...
  // Remove duplicates
  return users.filter(
    (user, index, array) => array.findIndex((u) => u._id === user._id) === index
  );
}
...
export const processCustomNotificationDelivery = action({
  args: {
  ...
  handler: async (ctx, { notificationIds, subject, message, channels }) => {
    // Process email and SMS delivery for custom notifications
    for (const notificationId of notificationIds) {
      ...
      const recipient = await ctx.runQuery(internal.userService.getUserById, {
      ...
      if (channels.includes('email') && recipient.email) {
        ...
        console.log(`📧 Custom email sent to ${recipient.email}: ${subject}`);
        await ctx.runMutation(internal.notifications.logEmailDelivery, {
      ...
      if (channels.includes('sms') && recipient.phone) {
        // In a real implementation, send actual SMS
        console.log(`📱 Custom SMS sent to ${recipient.phone}: ${message}`);
        await ctx.runMutation(internal.notifications.logSMSDelivery, {
          recipientId: recipient._id,
          phone: recipient.phone,
          message,
...
export const internal = {
  notifications: {
    ...
    processCustomNotificationDelivery,
    getNotificationById,
  },
};
